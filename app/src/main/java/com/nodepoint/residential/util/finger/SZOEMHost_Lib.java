...
public class SZOEMHost_Lib {
	...
	public SZOEMHost_Lib(Context context,IFingerState iFingerState) {
		m_bThreadWork = false;
		if(m_devComm == null){
        	m_devComm = new DevComm(context, m_IConnectionHandler,iFingerState);
        }
	...
	public int SZOEMHost_Lib_Init(Context context,IFingerState iFingerState)
	{
		m_bThreadWork = false;
		...
		if(m_devComm == null){
			...
			m_devComm.DevComm_Init(context, m_IConnectionHandler,iFingerState);
		}
	...
	*/
    ...
    public int OpenDevice()
    {
        if(m_devComm != null)
        {
            if (!m_devComm.IsInit())
            {
                if (!m_devComm.OpenComm())
                {
                	//m_txtStatus.setText("Failed init device!");
					Log.i("finglog:","Failed init device!");
                    return 1;
                }
            }
        	if (m_devComm.Run_TestConnection() ==  (short)DevComm.ERR_SUCCESS)
        	{
        		if (m_devComm.Run_GetDeviceInfo() == (short)DevComm.ERR_SUCCESS)
        		{
					...
					Log.i("finglog:","Open Device Success!");
					m_devComm.startScan();
        			return 0;
        		}
        		else
        		{
					...
					Log.i("finglog:","Can not connect to device!");
        			return 1;
        		}
        	}
        	else
