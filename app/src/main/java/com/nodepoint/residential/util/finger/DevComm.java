...
public class DevComm {
    ...
    public static final byte[] COMMAND_FINGER_CAPTRUED={(byte)0xAA,(byte)0x55,(byte)0x1A,(byte)0x01};
    public static final byte[] COMMAND_FINGER_RETRIEVED={(byte)0xA5,(byte)0x5A,(byte)0x1A,(byte)0x01};
...
// public static final int     CMD_GET_COMMNAD_VALID_FLAG_CODE			     = (0x013D);
    ...
    private IFingerState iFingerState=null;
    // USB
    private UsbController   m_usbBase;

	// Connection
	public byte m_nConnected;	// 0 : Not Connected,  2 : USB
	
	public Thread fingerScanThread=null;

    public DevComm(Context context, IUsbConnState usbConnState,IFingerState iFingerState){
    	
//    	LibDebugManage.DeleteLog();
    	
        mApplicationContext = context;
        this.iFingerState=iFingerState;
        // USB Init
    ...
    public int DevComm_Init(Context context, IUsbConnState usbConnState,IFingerState iFingerState)
    {
        this.iFingerState=iFingerState;
        m_nConnected = 0;
        return 0;
    ...
    public void startScan(){
        if(fingerScanThread==null){
            fingerScanThread=new Thread(new Runnable() {
                public void run() {
                    //Log.i("finglog:","sendCommandMessage------0!");
                    sendCommandMessage((short)CMD_FP_CANCEL_CODE,(short)0x00);
                    //Log.i("finglog:","sendCommandMessage------1!");
                    while(true){
                        //Log.i("finglog:","sendCommandMessage------2!");
                        sendCommandMessage((short)CMD_FINGER_DETECT_CODE,(short)0x00);
                        //Log.i("finglog:","sendCommandMessage------3!");
                        if(isFingerDetected()){
                            //Log.i("finglog:","sendCommandMessage------4!");
                            sendCommandMessage((short)CMD_FEATURE_OF_CAPTURED_FP_CODE,(short)0x00);
                            //Log.i("finglog:","sendCommandMessage------5!");
                            if(isFingerCaptured()){   //死1
                                Log.i("finglog:","isFingerCaptured to suee!");
                                //Log.i("finglog:","sendCommandMessage------6!");
                                retrieveFingerData();
                                //Log.i("finglog:","sendCommandMessage------7!");
                            }
                        }
                    }
                }
            });
            fingerScanThread.start();
        }
    }

    public void retrieveFingerData(){
       // Log.i("finglog:","retrieveFingerData------1!");
        try {
            USB_ReceiveDataPacket((short) CMD_FEATURE_OF_CAPTURED_FP_CODE);  //死掉部分
            //Log.i("finglog:","retrieveFingerData------2!");
            if(isHeadSame(m_abyPacket,COMMAND_FINGER_RETRIEVED)){
                Log.i("finglog:","isHeadSame to suee!");
                if(m_abyPacket.length>24) {
                    byte[] fingerData = new byte[570];
                    for (int i = 0; i < 570; i++) {
                        fingerData[i] = m_abyPacket[i + 8];
                    }
                    //Log.i("finglog:", "retrieveFingerData------3!");
                    onFinger(fingerData);
                    //Log.i("finglog:", "retrieveFingerData------4!");
                }
            }else{
                //Log.i("finglog:","retrieveFingerData------5!");
                Log.i("finglog:","isHeadSame  not to suee!");
            }
            //Log.i("finglog:","retrieveFingerData------6!");
        }catch (Exception e){
            int sdf= CMD_FEATURE_OF_CAPTURED_FP_CODE;
            System.out.print(sdf);
        }
    }

    public void onFinger(byte[] fingerData){
        if(iFingerState!=null){
            iFingerState.onFinger(fingerData);
        }
    }

    protected boolean isFingerCaptured(){
        return isHeadSame(m_abyPacket,COMMAND_FINGER_CAPTRUED);
    }

    protected boolean isFingerRetrieved(){
        return isHeadSame(m_abyPacket,COMMAND_FINGER_RETRIEVED);
    }

    protected boolean isFingerDetected(){
        return isHeadSame(m_abyPacket,COMMAND_FINGER_DETECT);
    }
...
//    		return false;
    ...
    boolean USB_ReceiveDataAck(short p_wCmd)
    {
    	...
    	do 
    	{
            ...
            //Log.i("zhlerr----","4-3");
            //Log.i("zhlerr----","4-4-m_abyPacket--"+m_abyPacket[0]+"------"+m_abyPacket[1]+"----"+byteArrayToHexStr(m_abyPacket));
            //Log.i("zhlerr----","4-4-w_WaitPacket--"+byteArrayToHexStr(w_WaitPacket));

            //处理AFAFAFAF问题
            if(m_abyPacket[0]==-81 && m_abyPacket[1]==-81){
                //Log.i("zhlerr----","4-5");
                return false;
            }
    ...
    public boolean USB_DownImage(byte[] pBuf, int nBufLen)
    {
    	...
    	while (w_nRemainCount > ONCE_UP_IMAGE_UINT)
    	{
    		...
    		if (!m_usbBase.UsbSCSIRead(btCDB, 6, w_pImgBuf,ONCE_UP_IMAGE_UINT, SCSI_TIMEOUT))
    			return false;
    ...
    public boolean memcmp(byte[] p1, byte[] p2, int nLen)
    {
    	...
    	{
    		if (p1[i] != p2[i]) {
                //Log.i("zhlerr****","****"+p1[i] +"****"+p2[i] );  //zhl err
                return false;
            }else{
                //Log.i("zhlerrnew****","****"+p1[i] +"****"+p2[i] );  //zhl err
            }
